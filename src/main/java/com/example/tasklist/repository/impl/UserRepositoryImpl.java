...
public class UserRepositoryImpl implements UserRepository {
    ...
    public Optional<User> findByUsername(String username) {
        try {
            final Connection connection = dataSourceConfig.getConnection();
            final PreparedStatement statement = connection.prepareStatement(FIND_BY_USERNAME,
                    ResultSet.TYPE_SCROLL_INSENSITIVE,
                    ResultSet.CONCUR_READ_ONLY);
    ...
    public void create(User user) {
        try {
            final Connection connection = dataSourceConfig.getConnection();
            final PreparedStatement statement = connection.prepareStatement(CREATE, PreparedStatement.RETURN_GENERATED_KEYS);
            statement.setString(1, user.getName());
            statement.setString(2, user.getUsername());
            statement.setString(3, user.getPassword());
            statement.executeUpdate();
    ...
    public void insertUserRole(Long userId, Role role) {
        try {
            final Connection connection = dataSourceConfig.getConnection();
            final PreparedStatement statement = connection.prepareStatement(INSERT_USER_ROLE);
    ...
    public boolean isTaskOwner(Long userId, Long taskId) {
        try {
            final Connection connection = dataSourceConfig.getConnection();
            final PreparedStatement statement = connection.prepareStatement(IS_TASK_OWNER);
            statement.setLong(1, userId);
